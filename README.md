...
word revealed at round-end, **with each stroke's color** — straight into a
**PostgreSQL** database (via the **Drizzle ORM**). A whole fleet of bot containers
writes concurrently to one `drawings` table (no shared-file corruption), and the
**dashboard** reads from it and pushes live updates over a **WebSocket** — so it
stays snappy no matter how large the dataset grows. (No `DATABASE_URL`? It
transparently falls back to `data/harvest/*.ndjson` files for quick local runs.)
...
```
 bot (Bun)  ──harvest (word, strokes, colors)──▶  PostgreSQL  ◀── dashboard (WebSocket)
     ▲                                            (Drizzle ORM)      │
...
Compose brings up **Postgres** + a one-shot **`migrate`** service (creates the
schema and imports any pre-existing `data/harvest/*.ndjson` on first run), then the
`bot`, `trainer`, and `dashboard`. They share `./data` (models, caches) and one
database: the bot harvests + hot-reloads, the trainer (GPU) retrains as the harvest
grows, and the dashboard serves live stats from Postgres over a WebSocket
(<http://localhost:28080>). The trainer reaches host Ollama via
...
To scale the harvest fleet (all writing to the same DB, concurrent-safe):

```sh
docker compose -f docker-compose.harvest.yml up --build --scale harvester=4
```

Override DB creds with `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB`.
Migrations live in [`db/migrations/`](db/migrations); regenerate after a schema
change with `bun run db:generate`, apply with `DATABASE_URL=… bun run db:migrate`.
...
train/            PyTorch pipeline (GPU) — harvest → clean → train → ONNX
  ...
  dataset.py      load samples (Postgres, or ndjson fallback) → tensors + open-set vocab
...
data/
  ...
  drawable_words.txt
  harvest/        legacy ndjson shards (fallback when no DATABASE_URL; gitignored)
  model/          trained detector.onnx + vocab (generated; gitignored)
db/               PostgreSQL backend (Drizzle ORM)
  schema.js       drawings + bots tables
  index.js        connection (postgres.js + Drizzle) — optional via DATABASE_URL
  queries.js      data access (insert, word counts, pagination, bot upserts)
  migrate.js      apply migrations (+ optional ndjson import) — the migrate container
  import-ndjson.js  one-shot legacy ndjson → Postgres
